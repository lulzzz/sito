    ...
    public sealed class Promise
    {
        ...
        public PromiseState State
        {
            get
            {
                ...
                return statusToState(Task.Status);
            }
        }

        [Hidden]
        public Task<JSValue> Task { get; }

        [Hidden]
        public bool Complited => Task.IsCompleted;

        [Hidden]
        public JSValue Result => Task.Status == TaskStatus.RanToCompletion ?
            Task.Result
            :
        ...
        public Promise then(Func<JSValue, JSValue> onFulfilment, Func<JSValue, JSValue> onRejection)
        {
            ...
            var thenTask = onFulfilment == null ? null : Task.ContinueWith(task => onFulfilment(Result), TaskContinuationOptions.OnlyOnRanToCompletion);
