...
namespace JS.Core.Expressions
{
    ...
    public class Assignment : Expression
    {
        ...
        {
            ExceptionHelper.ThrowTypeError(string.Format(Messages.CannotAssignReadOnly, _left));
        }
        ...
        public override void Optimize(ref CodeNode _this, FunctionDefinition owner, InternalCompilerMessageCallback message, Options opts, FunctionInfo stats)
        {
            ...
            if (vr != null)
            {
                ...
                else message?.Invoke(MessageLevel.CriticalWarning, Position, Length, "Assign to undefined variable \"" + vr.Name + "\". It will declare a global variable.");
            }
            ...
            if (variable != null && variable._descriptor.IsDefined && (_codeContext & CodeContext.InWith) == 0 && !variable._descriptor.captured)
            {
                if (!stats.ContainsEval && !stats.ContainsWith)
                {
                    ...
                    {
                        if ((owner.Body._strict || variable._descriptor.owner != owner || !owner.FunctionInfo.ContainsArguments)) // аргументы это одна сущность с двумя именами
                        {
                            bool last = true;
