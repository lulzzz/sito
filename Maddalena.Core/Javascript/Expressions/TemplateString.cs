...
namespace Maddalena.Core.Javascript.Expressions
{
    public sealed class TemplateString : Expression
    {
        ...
        public IEnumerable<string> Messages => strings;
        public IEnumerable<Expression> Expressions => expressions;
        public TemplateStringMode Mode { get; private set; }

        protected internal override bool ContextIndependent => expressions.All(t => t.ContextIndependent);
        ...
        {
            if (state.Code[index] != '`')
                ExceptionHelper.ThrowSyntaxError("UnexpectedToken", state.Code, index);
            ...
            for (;;)
            {
                ...
                if (state.Code[pos] == '$')
                {
                    ...
                    if (state.Code[pos] != '}')
                        ExceptionHelper.ThrowSyntaxError("UnexpectedToken", state.Code, pos);
                }
        ...
        public override void RebuildScope(FunctionInfo functionInfo, Dictionary<string, VariableDescriptor> transferedVariables, int scopeBias)
        {
            foreach (var exp in expressions)
                exp.RebuildScope(functionInfo, transferedVariables, scopeBias);
        }
