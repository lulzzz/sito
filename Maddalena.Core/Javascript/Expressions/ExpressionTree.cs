    ...
    public sealed class ExpressionTree : Expression
    {
            ...
            while (cur != null)
            {
                ...
                for (; types.Count > 0;)
                {
                    ...
                    if (((topType & (int)OperationTypeGroups.Special) > ((int)cur.Type & (int)OperationTypeGroups.Special))
                        || (((topType & (int)OperationTypeGroups.Special) == ((int)cur.Type & (int)OperationTypeGroups.Special))
        ...
        private static Expression parseContinuation(ParseInfo state, Expression first, int startIndex, ref int i, ref bool root, bool forUnary, bool processComma, bool forNew, bool forForLoop)
        {
            ...
            if ((kind == OperationType.Assignment) || assign)
            {
                ...
                if (error)
                    ExceptionHelper.ThrowReferenceError(Messages.InvalidLefthandSideInAssignment, state.Code, first.Position, first.Length);
            }
