    ...
    public class JSObject : JSValue
    {
        ...
        public sealed override JSObject __proto__
        {
            ...
            get
            {
                ...
                if (_objectPrototype == null || _objectPrototype._valueType < JSValueType.Object)
                    _objectPrototype = GetDefaultPrototype() ?? @null;

                return _objectPrototype._oValue == null ? @null : _objectPrototype;
            }
            [Hidden]
            set
            {
                ...
                {
                    ((JSObject) _oValue).__proto__ = value;
        ...
        protected internal override JSValue GetProperty(JSValue key, bool forWrite, PropertyScope propertyScope)
        {
            ...
            else
            {
                ...
                var fromProto =
                    (propertyScope >= PropertyScope.Super || _fields == null || !_fields.TryGetValue(name, out res) ||
                     res._valueType < JSValueType.Undefined) && (proto = __proto__)._oValue != null;
            ...
            if (members._valueType >= JSValueType.Object)
                foreach (var item in members)
                {
                    ...
                    if (value.Exists && (get.Exists || set.Exists))
                        ExceptionHelper.Throw(
                            new TypeError("Property can not have getter or setter and default value."));
        ...
        private static JSObject definePropertyImpl(JSObject target, JSObject desc, string memberName)
        {
                ...
                if (memberName == "length")
                    try
                    {
                        if (value.Exists)
                        {
                            ...
                            if (!((Array) target).SetLenght(nlen))
                                ExceptionHelper.Throw(
            ...
            if (value.Exists)
            {
                if (!config
                    ...
                    ExceptionHelper.Throw(
                        new TypeError("Cannot change value of not configurable not writable peoperty."));
