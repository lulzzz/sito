...
namespace JS.Core.Expressions
{
    public sealed class ObjectDesctructor : Expression
    {
        ...
        public static bool CheckObjectDefinition(ArrayDefinition arrayDefinition, bool @throw)
        {
            ...
            if (arrayDefinition.Elements.All(ExpressionTree.canBeAssignee)) return true;
            if (@throw)
        ...
        public static bool CheckObjectDefinition(ObjectDefinition objectDefinition, bool @throw)
        {
            ...
            if (objectDefinition.Values.Any(t => !ExpressionTree.canBeAssignee(t)))
            {
                if (@throw)
                    ExceptionHelper.ThrowReferenceError(Messages.InvalidLefthandSideInAssignment);
                return false;
            ...
            if (objectDefinition.ComputedProperties.Any(t => !ExpressionTree.canBeAssignee(t.Value)))
            {
                if (@throw)
                    ExceptionHelper.ThrowReferenceError(Messages.InvalidLefthandSideInAssignment);
                return false;
        ...
        private static void collectTargetVariables(ObjectDefinition objectDefinition, List<Variable> result)
        {
            ...
            foreach (var definition in objectDefinition.Values)
            {
                if (definition is Variable)
                {
                    result.Add((Variable)definition);
                }
                else
                {
                    collectTargetVariables(definition as ObjectDefinition, result);
                    collectTargetVariables(definition as ArrayDefinition, result);
                }
            }

            foreach (var prop in objectDefinition.ComputedProperties)
            {
                if (prop.Value is Variable variable)
                {
                    result.Add(variable);
                }
                else
                {
                    collectTargetVariables(prop.Value as ObjectDefinition, result);
                    collectTargetVariables(prop.Value as ArrayDefinition, result);
                }
            }
        }
