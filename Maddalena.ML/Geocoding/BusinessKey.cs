...
namespace Maddalena.ML.Geocoding
{
    ...
    public class BusinessKey
    {
            ...
            set
            {
                ...
                if (Regex.IsMatch(formattedChannel, @"^[a-z_0-9.-]+$"))
                    ...
                    throw new ArgumentException(
                        "Must be an ASCII alphanumeric string; can include a period (.), underscore (_) and hyphen (-) character",
                        "channel");
            }
        }

        public bool HasChannel => !string.IsNullOrEmpty(Channel);
        ...
        public string GenerateSignature(string url)
        {
            ...
            var usablePrivateKey = SigningKey.Replace("-", "+").Replace("_", "/");
