...
namespace Maddalena.Markdig.Syntax
{
    ...
    public class LinkReferenceDefinition : LeafBlock
    {
        ...
        public delegate Inline CreateLinkInlineDelegate(InlineProcessor inlineState, LinkReferenceDefinition linkRef,
            Inline child = null);
        ...
        public static bool TryParse<T>(ref T text, out LinkReferenceDefinition block) where T : ICharIterator
        {
            ...
            if (!LinkHelper.TryParseLinkReferenceDefinition(ref text, out label, out url, out title, out labelSpan,
                out urlSpan, out titleSpan))
            {
                return false;
            }
