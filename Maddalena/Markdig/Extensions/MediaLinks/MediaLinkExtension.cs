...
namespace Markdig.Extensions.MediaLinks
{
    ...
    public class MediaLinkExtension : IMarkdownExtension
    {
        ...
        private bool TryRenderIframeFromKnownProviders(Uri uri, HtmlRenderer renderer, LinkInline linkInline)
        {
            var foundProvider =
                KnownHosts
                    .Where(pair => uri.Host.StartsWith(pair.HostPrefix,
                        StringComparison.OrdinalIgnoreCase)) // when host is match
                    .Select(provider =>
                        new
                        {
            ...
            var htmlAttributes = GetHtmlAttributes(linkInline);
            renderer.Write(
                $"<div class=\"embed-responsive embed-responsive-16by9\"><iframe src=\"{foundProvider.Result}\"");
