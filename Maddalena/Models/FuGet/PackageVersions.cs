...
namespace Maddalena.Models.FuGet
{
    ...
    {
        private static readonly PackageVersionsCache cache = new PackageVersionsCache();
        ...
        public PackageVersion GetVersion(object inputVersion)
        {
            ...
            var v = Versions.FirstOrDefault(x => x.VersionString == version) ?? Versions.LastOrDefault();
            if (v == null)
        ...
        private void Read(JArray items)
        {
            foreach (var v in items)
            {
                var version = v["version"]?.ToString();
                if (version == null) version = v["catalogEntry"]?["version"]?.ToString();
                if (version != null) Versions.Add(new PackageVersion {VersionString = version});
            }
        }

        private class PackageVersionsCache : DataCache<string, PackageVersions>
        {
