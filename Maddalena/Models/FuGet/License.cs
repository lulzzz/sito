...
namespace Maddalena.Models.FuGet
{
    ...
    {
        private static readonly List<License> known = new List<License>();
        ...
        private void LoadTemplate()
        {
            using (var s = GetType().Assembly
                .GetManifestResourceStream($"FuGetGallery.Resources.Licenses.{TemplateName}.txt"))
            {
                if (s == null) throw new Exception("Missing license " + TemplateName);
        ...
        private static HashSet<string> BuildBigramSet(string input)
        {
            ...
            for (var i = 0; i < input.Length - 1; i++) bigrams.Add(input.Substring(i, 2));
            return bigrams;
        }
        ...
        {
            var intersection = new HashSet<string>(x);
